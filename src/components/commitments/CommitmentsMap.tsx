...
export function CommitmentsMap({
  recurrings,
  variableEstimate,
  year,
  month,
  ...
  const navigateMonth = (delta: number) => {
    ...
    startTransition(() => {
      router.push(
        `/mapa-financeiro?year=${next.year}&month=${next.month}`,
        { scroll: false },
      );
    });
  };
              ...
              {selectedDay.events.map((event) => (
                ...
                >
                  <span className="flex min-w-0 items-center gap-2">
                    <span
                      className={cn(
                        ...
                        event.type === "INCOME" ? "bg-income" : "bg-expense",
                      )}
                      style={
                        event.type === "EXPENSE"
                  ...
                  <span
                    className={cn(
                      "font-semibold",
                      event.type === "INCOME" ? incomeClass() : expenseClass(),
                    )}
                  >
