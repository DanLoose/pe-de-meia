...
- **Next.js app**: `npm run dev` serves on `http://localhost:3000`. The app is auth-gated; `/` redirects to `/login` when logged out, or `/mapa-financeiro` when logged in. After sign-in the app redirects to `/mapa-financeiro`. Demo login: `demo@pedemeia.dev` / `password123`.
...
- `npm run test:e2e` — prefer running against a seeded demo user; auth lands on `/mapa-financeiro`.
